...
import { HuggingFaceEmbeddings } from "./huggingface-embeddings"; // Lightweight HuggingFace embeddings
import { ChatGroq } from "@langchain/groq"; // Groq's Llama AI model
...
export async function callAgent(
  ...
  try {
    ...
    // @ts-ignore - Type instantiation error due to version mismatch
    const itemLookupTool = tool(
      ...
      async ({ query, n = 10 }) => {
        try {
          ...
          // Create vector store instance for semantic search using lightweight HuggingFace embeddings
          const vectorStore = new MongoDBAtlasVectorSearch(
            new HuggingFaceEmbeddings(), // Lightweight HuggingFace embeddings (384 dimensions)
            dbConfig
          );
    ...
    // Initialize the AI model (Groq's Llama)
    const model = new ChatGroq({
      model: "llama-3.3-70b-versatile", // Use Llama 3.3 70B model via Groq
      temperature: 0, // Deterministic responses (no randomness)
      maxRetries: 0, // Disable built-in retries (we handle our own)
      apiKey: process.env.GROQ_API_KEY, // Groq API key from environment
