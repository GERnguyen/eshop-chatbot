...
async function seedDatabase() {
  ...
  try {
    ...
    if (!HF_API_KEY) {
      throw new Error("HUGGINGFACEHUB_API_KEY is required for generating embeddings")
    }
    ...
    for (let i = 0; i < sampleFurniture.length; i++) {
      ...
      console.log(`Processing item ${i + 1}/${sampleFurniture.length}: ${item.item_name}`)
    ...
    try {
      ...
      console.log("Note: Vector index creation may require Atlas M10+ cluster or may already exist")
