...
async function startServer() {
  try {
    ...
    // Define endpoint to get all products (GET /products)
    app.get('/products', async (req: Request, res: Response) => {
      try {
        const db = client.db("inventory_database")
        const collection = db.collection("items")
        // Get all products, exclude embedding field (too large)
        const products = await collection.find({}, { 
          projection: { embedding: 0, embedding_text: 0 } 
        }).toArray()
        res.json(products)
      } catch (error) {
    ...
    // Define endpoint to get single product (GET /products/:id)
