// Import Google's Gemini chat model for AI text generation
import { ChatGoogleGenerativeAI } from "@langchain/google-genai"
// Import lightweight HuggingFace embeddings (direct API calls)
import { getHuggingFaceEmbedding } from "./huggingface-embeddings"
...
async function seedDatabase(): Promise<void> {
  try {
    ...
    for (let i = 0; i < recordsWithSummaries.length; i++) {
      ...
      // Generate embedding using lightweight HuggingFace API
      const embedding = await getHuggingFaceEmbedding(record.pageContent)
      
      // Insert document with embedding directly
      await collection.insertOne({
        embedding_text: record.pageContent,
        embedding: embedding,
        ...record.metadata,
      })

      // Log progress for each successfully processed item
      console.log(`Successfully processed & saved record (${i + 1}/${recordsWithSummaries.length}):`, record.metadata.item_id)
